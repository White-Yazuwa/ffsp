...
if (Config.LANG == 'SI') {
    var PB_DESC = "ඔබේ පැතිකඩ චරිතාපදානය වෙනස් කරන්න"
    var PN_DESC = "ඔබේ පැතිකඩ නම වෙනස් කරන්න"
    var NEED_WD = "කරුණාකර වචනය අවශ්‍යයි !!"
    var SET_UP = "*⚙️පැතිකඩ ජෛව සැකසීම...*"
    var SET_NAME = "*⚙️පැතිකඩ නම සැකසීම...*"       
     }
else {
    var PB_DESC = "Change Your Profile Bio"
    var PN_DESC = "Change Your Profile Name"
    var NEED_WD = "Please Need Word !!"
    var SET_UP = "*⚙️ Setting Up Profile Bio...*"
    var SET_NAME = "*⚙️ Setting Up Profile Name...*"
     }
...
cobra.addCommand({pattern: 'setbio ?(.*)', fromMe: true, desc: PB_DESC, dontAddCommandList: true}, (async (message, match) => {    
    const pname = match[1]
...
cobra.addCommand({pattern: 'setname ?(.*)', fromMe: true, desc: PN_DESC, dontAddCommandList: true}, (async (message, match) => {    
    const pname = match[1]
...
var SUCC_WELBG = ''
if (Config.LANG == 'SI') SUCC_WELBG  = '✅ Welcome Backgroud සාර්ථකව සකසා ඇත'
if (Config.LANG == 'EN') SUCC_WELBG  = '✅ Successfully Setted Welcome Background'
var NEED_PHOTO = ''
...
cobra.addCommand({pattern: 'setwelcomebg$', fromMe: true, desc: PP_DESC, dontAddCommandList: true}, (async (message, match) => {    
...
await heroku.patch(baseURI + '/config-vars', { 
    ...
    await message.client.sendMessage(message.jid, '*' + SUCC_WELBG + '*', MessageType.text)
    })
}));
