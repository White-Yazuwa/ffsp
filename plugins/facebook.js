...
Aqua.addCommand({ pattern: 'fb ?(.*)', fromMe: wk, desc:Lang.FB_DESC, deleteCommand: false }, async (message, match) => {
  ...
  await axios.get(`https://sanuw-api.herokuapp.com/docs/download/facebook?url=${fblink}&apikey=sanuwa`).then(async (response) => {
    ...
    const msg = '┌───[🐋𝙰𝚀𝚄𝙰𝙱𝙾𝚃🐋]\n\n  *📥FACEBOOK DOWNLODER*\n\n│🎭ᴜᴘʟᴏᴀᴅᴇʀ: ' + response.data.result.data.author + '\n\n│🎪ᴛɪᴛʟᴇ: ' + response.data.result.data.title + '\n\n└───────────◉'
     const viddata = await axios.get(response.data.result.data.HD, { responseType: 'arraybuffer'}); 
