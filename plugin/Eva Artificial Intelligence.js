...
if (conf.LANG == 'EN') vtalk_dsc = 'Fulleva හ voice කතාබහට පටන් ගනී.', reply_eva = '*ඕනෑම Voice පණිවිඩයකට පිළිතුරු දෙන්න!*'
...
cobra.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
...
cobra.addCommand({ pattern: 'vtalk$', desc: vtalk_dsc,dontAddCommandList: true, fromMe: wk }, (async (message, match) => {
...
if (conf.LANG == 'EN') {
    ...
    already_on = 'fulleva artificial intelligence is already fully functional.'
    already_off = 'fulleva artificial intelligence is currently running semi-functional.'
    succ_on = 'fulleva Opened Fully Functionally! Please wait a bit! ✅'
    succ_off = 'fulleva Set to Semi-Functional! Please wait a bit! ☑️'
}
if (conf.LANG == 'SI') {
    fulleva_dsc = 'පූර්ණ ක්‍රියාකාරී Fulleva විශේෂාංග සක්‍රීය කරයි. ඔබේ ගිණුම AI බොට් එකක් බවට පත් කරන්න!'
    already_on = 'Fulleva කෘතිම බුද්ධිය දැනටමත් සම්පුර්ණයෙන්ම ක්‍රියාත්මකයි.'
    already_off = 'Fulleva කෘතීම බුද්ධිය දැනට අර්ධ ක්‍රියාකාරී ලෙස ක්‍රියාත්මක වේ.'
    succ_on = 'Fulleva සම්පූර්ණයෙන්ම ක්‍රියාකාරීව විවෘතයි! කරුණාකර ටිකක් ඉන්න! ✅'
    succ_off = 'Fulleva අර්ධ ක්‍රියාකාරී ලෙස සකසන්න! කරුණාකර ටිකක් ඉන්න! ☑️'
}

cobra.addCommand({ pattern: 'fulleva ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.fulleva on / off' }, (async (message, match) => {
